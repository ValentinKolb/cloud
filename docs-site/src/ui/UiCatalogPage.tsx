  ...
  return (
    <section class="ui-example" id={props.id}>
      ...
      <div class="ui-example-demo">
        <div class="ui-preview">{props.children as never}</div>
        <pre class="ui-code">
      ...
      <main class="ui-catalog">
        <header class="ui-intro">
          <div>
            ...
            <h1>{props.focus ? componentName(props.focus) : "Components that applications actually share."}</h1>
          </div>
          <p>
            This catalog renders the components from <code>@valentinkolb/cloud/ui</code>. It is both documentation and a
            development surface: changes to the package appear here without copying a component into the website.
          </p>
        </header>
        <Show when={!props.focus}>
          <nav class="ui-index" aria-label="Component index">
            <a href="/ui/panel-header">PanelHeader</a>
            <a href="/ui/status-badge">StatusBadge</a>
            <a href="/ui/stat-grid">StatGrid</a>
          </nav>
        </Show>
