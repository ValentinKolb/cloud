...
| List/detail app | `AppWorkspace.Content` with Main plus `AppWorkspace.Detail`; URL-backed selection | `packages/contacts/src/frontend/page.tsx`, `packages/contacts/src/frontend/[bookId]/page.tsx` | Local-only selected id that breaks reload/share/back; hand-written second column inside Main |
| Secondary workspace drawer | `AppWorkspace.BottomDrawer` with a stable purpose-based id | UI Lab `/app/ui-lab/layout/workspace` | Navigation in a drawer; duplicated detail content; entity-specific persistence ids |
...
- Contextual right-side detail is an `AppWorkspace.Detail` sibling after Main inside `AppWorkspace.Content`, never a second grid or flex column inside Main. Detail panels use a `detail-stack` wrapper with separate `detail-section` cards. The stack owns inter-card spacing; sections own only their surface and inner padding.
- Multiple details use stable semantic ids and appear only when distinct contexts must remain visible together. A detail does not collapse to an icon rail by default; resize or close it. On mobile, keep at most one open detail.
- Secondary activity/log/preview/composer content below Main and Details uses `AppWorkspace.BottomDrawer`. Use `Panes` instead only for app-owned IDE/editor/query splits inside Main.
