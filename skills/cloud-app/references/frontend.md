...
For new built-in apps, start with `design.md` and `app-ui-patterns.md` before this file. `design.md` defines visual and interaction rules, `app-ui-patterns.md` selects the Cloud shell, and this file documents component APIs and implementation patterns.
...
Tabbed settings dialogs must use the shared `dialog-fixed-frame` height. It keeps the stable desktop proportion while clamping to the dynamic mobile viewport and safe areas. `SettingsModal` then keeps the rail and outer frame stable while only the active pane scrolls. Apply the same class to loading and error fallbacks so lazy loading does not resize the modal. A `max-height` alone is insufficient because shorter tabs still collapse the frame.
...
Use `panelDialogFixedOptions` for `PanelDialog` editors whose tabs, modes, or progressive sections can substantially change content height. It keeps the standard dialog width, fixes the outer height to the shared viewport-safe frame, and lets `PanelDialog.Body` scroll. Keep `panelDialogOptions` for content-sized single-view dialogs and `panelDialogWorkspaceOptions` for full workspace-sized editors.
