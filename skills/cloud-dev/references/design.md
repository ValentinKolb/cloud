...
- Sidebar, MainPane, Detail, and BottomDrawer share one resize controller and one visual affordance: a generous invisible hit target with a one-pixel full-edge guide plus a short centered handle on hover, focus, and drag. Sidebar and MainPane also show a subtle shadow toward the adjacent work region by default; disable it only with `resizeShadow={false}` when the surrounding composition already supplies equivalent depth. Geometry uses stable purpose-based ids and the per-app SSR cookie. Never implement a local resize handle for one app.
...
- Help does not use a floating utility window. Its window action opens the
  shared, reload-safe full-page Help view in a normal browser tab or window so
  navigation, refresh, copy-as-Markdown, and deep links remain predictable.
...
The git hygiene and pre-handoff sweep that accompany these rules live in `checklist.md`.
