...
1. **Resolve every request through `actor` and `accessSubject` — not `c.get("user")`.** `c.get("user")` is the pre-service-account compatibility path. It is *typed* `User` but is `undefined` at runtime for a resource-bound service account, so any check written against it silently excludes API keys and OAuth service tokens while still compiling. The related trap: never authorize from `User.memberofGroupIds`, which is display metadata the access helpers ignore. → `auth.md`
