...
export const accountLifecycle = {
    ...
    for (const row of rows) {
      ...
      try {
        ...
        try {
          ...
          log.warn("Failed to revoke sessions after automatic IPA demotion", {
            userId,
            uid,
    ...
    for (const row of rows) {
      ...
      try {
        ...
        try {
          ...
          log.warn("Failed to revoke sessions after expired guest cleanup", {
            userId,
            uid,
            error: error instanceof Error ? error.message : String(error),
          });
    ...
    for (const row of rows) {
      ...
      try {
        ...
        try {
          ...
          log.warn("Failed to revoke sessions after expired local user cleanup", {
            userId,
            uid,
            error: error instanceof Error ? error.message : String(error),
          });
  ...
  listDeletedAccounts: async (config: { page: number; perPage: number; reason?: string; search?: string }) => {
    ...
    const rows = await sql<DbRow[]>`
      SELECT id, deleted_user_id, uid, mail, display_name, previous_realm, reason, deleted_at, meta
      FROM auth.deleted_accounts
      WHERE (${reason}::text IS NULL OR reason = ${reason})
        AND (
          ${pattern}::text IS NULL
