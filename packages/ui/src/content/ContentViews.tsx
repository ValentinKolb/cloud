...
export function LogEntriesTable(props: LogEntriesTableProps): JSX.Element {
  return (
    <div class="k2b-log-list">
      <For each={props.entries} fallback={<div class="k2b-log-list__empty">{props.empty ?? "No log entries"}</div>}>
        {(entry) => (
          <button type="button" class="k2b-log-entry" data-level={entry.level} onClick={() => props.onSelect?.(entry)}>
            <time dateTime={new Date(entry.timestamp).toISOString()}>{new Date(entry.timestamp).toLocaleString()}</time>
            <span class="k2b-log-entry__level">{entry.level}</span>
            <Show when={entry.source}>
              <span class="k2b-log-entry__source">{entry.source}</span>
            </Show>
  ...
  return (
    <div class={`k2b-docs ${props.class ?? ""}`}>
      <Show when={props.navigation}>
        <nav class="k2b-docs__navigation">{props.navigation}</nav>
      </Show>
      <main class="k2b-docs__content">{props.children}</main>
      <Show when={props.aside}>
        <aside class="k2b-docs__aside">{props.aside}</aside>
      </Show>
    </div>
  );
}
