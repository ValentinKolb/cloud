...
describe("@k2b/ui Panes and SettingsModal behavior", () => {
  ...
  test("shows every valid target and drops through the selected group intent", async () => {
    const dom = createDomTestHarness();
    const { default: Panes } = await import("../src/layout/Panes");
    const [value, setValue] = createSignal<PanesValue>({
      version: PANES_VALUE_VERSION,
      root: {
        ...
        sizes: [50, 50],
        children: [
          { type: "leaf", id: "source-leaf", elementIds: ["source"], activeElementId: "source", presentation: "single" },
          { type: "leaf", id: "preview-leaf", elementIds: ["preview"], activeElementId: "preview", presentation: "single" },
        ],
      },
    });
        ...
        createComponent(Panes.Root, {
          ...
          get children() {
            return ["source", "preview"].map((id) =>
              createComponent(Panes.Element, { id, title: id === "source" ? "Source" : "Preview", children: id }),
            );
          },
        }),
      dom.root,
    ...
    if (leaves.length !== 2 || headers.length !== 2) throw new Error("Expected two pane groups");
    leaves[0]!.getBoundingClientRect = () => rect(0, 0, 300, 180);
    leaves[1]!.getBoundingClientRect = () => rect(0, 180, 300, 180);
    headers[0]!.getBoundingClientRect = () => rect(0, 0, 120, 40);
    headers[1]!.getBoundingClientRect = () => rect(0, 180, 120, 40);
    const source = headers[0]!.querySelector<HTMLElement>("[data-dnd-draggable]");
    if (!source) throw new Error("Expected draggable Source tab");
    ...
    expect(targets.some((target) => target.dataset.kind === "tab")).toBe(true);
    ...
    expect(targets.some((target) => target.dataset.disabled === "true")).toBe(true);

    const bottomTarget = leaves[1]!.querySelector<HTMLElement>('.k2b-panes__drop-target[data-zone="bottom"]:not([data-disabled])');
    const leftTarget = leaves[1]!.querySelector<HTMLElement>('.k2b-panes__drop-target[data-zone="left"]:not([data-disabled])');
    if (!bottomTarget || !leftTarget) throw new Error("Expected enabled trapezoid targets");
    bottomTarget.getBoundingClientRect = () => rect(0, 312, 300, 48);
    leftTarget.getBoundingClientRect = () => rect(0, 220, 48, 140);
    ...
    await Promise.resolve();
    expect(bottomTarget.dataset.active).toBe("true");
    expect(leftTarget.dataset.active).toBeUndefined();
    ...
    await Promise.resolve();
    expect(leftTarget.dataset.active).toBe("true");
    expect(bottomTarget.dataset.active).toBeUndefined();

    const groupTarget = leaves[1]!.querySelector<HTMLElement>('.k2b-panes__drop-target[data-kind="group"]');
    if (!groupTarget) throw new Error("Expected explicit target-group drop target");
  ...
  test("uses explicit tab slots and hides disabled split directions", async () => {
    const dom = createDomTestHarness();
    const { default: Panes } = await import("../src/layout/Panes");
    const [value, setValue] = createSignal<PanesValue>({
      version: PANES_VALUE_VERSION,
    ...
    const dispose = render(
      () =>
        createComponent(Panes.Root, {
          ...
          get children() {
            return ["one", "two"].map((id) => createComponent(Panes.Element, { id, title: id, children: id }));
          },
        }),
      dom.root,
    );
    ...
    const tabs = Array.from(dom.root.querySelectorAll<HTMLElement>('[role="tab"]'));
    const source = tabs[1]?.querySelector<HTMLElement>("[data-dnd-draggable]");
    if (!source) throw new Error("Expected draggable second tab");
    tabs.forEach((tab, index) => {
      tab.getBoundingClientRect = () => rect(index * 120, 0, 120, 40);
    });
    source.getBoundingClientRect = () => rect(120, 0, 120, 40);
    ...
    expect(dom.root.querySelector('[data-zone="left"], [data-zone="right"]')).toBeNull();
    expect(dom.root.querySelector('[data-zone="top"]')).not.toBeNull();
    expect(dom.root.querySelector('[data-zone="bottom"]')).not.toBeNull();
    expect(dom.root.querySelector('[data-kind="group"]')).toBeNull();

    const tabTarget = tabs[0]!.querySelector<HTMLElement>('.k2b-panes__drop-target[data-kind="tab"]');
    if (!tabTarget) throw new Error("Expected explicit leading tab slot");
    tabTarget.getBoundingClientRect = () => rect(0, 0, 32, 40);
    dom.window.dispatchEvent(
      new dom.window.PointerEvent("pointermove", {
        clientX: 16,
        clientY: 20,
    ...
    await Promise.resolve();
    expect(tabTarget.dataset.active).toBe("true");
    ...
    await Promise.resolve();
    expect(value().root).toMatchObject({ elementIds: ["two", "one"], activeElementId: "two" });
