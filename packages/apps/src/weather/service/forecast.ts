...
export const getCurrentWeather = async (config?: ForecastLocationConfig): Promise<CurrentWeather | null> => {
  const lat = config?.lat ?? getSync<string>("weather.default_lat");
  const lon = config?.lon ?? getSync<string>("weather.default_lon");
  ...
  const ttl = Math.round((getSync<number>("weather.cache_minutes") ?? 30) * 60);
