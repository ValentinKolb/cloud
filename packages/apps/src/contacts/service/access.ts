...
/**
 * Returns all access IDs attached to one book. Used by cleanup/diagnostic flows.
 */
export const listBookAccessIds = async (bookId: string): Promise<string[]> => {
  if (!isUuid(bookId)) return [];
...
/**
 * Loads access IDs for multiple books in one query.
 */
export const listBookAccessIdsByBooks = async (bookIds: string[]): Promise<Map<string, string[]>> => {
  const validBookIds = bookIds.filter(isUuid);
  if (validBookIds.length === 0) return new Map();

  const rows = await sql<{ book_id: string; access_id: string }[]>`
    SELECT book_id, access_id
    FROM contacts.book_access
    WHERE book_id = ANY(${toPgUuidArray(validBookIds)}::uuid[])
  `;

  const grouped = new Map<string, string[]>();
  for (const row of rows) {
    grouped.set(row.book_id, [...(grouped.get(row.book_id) ?? []), row.access_id]);
  }
  return grouped;
};
