...
import { apiClient } from "@valentinkolb/cloud-core/api/admin-account-lifecycle-client";
...
const OPERATIONS: readonly OperationConfig[] = [
  {
    ...
    label: "Force Sync",
    icon: "ti ti-refresh",
    redirectTo: "/admin/logs?source=auth:ipa:sync",
  ...
  },
  {
    ...
    redirectTo: "/admin/logs?source=auth:ipa:backfill",
  ...
  },
  {
    ...
    icon: "ti ti-user-exclamation",
    redirectTo: "/admin/logs?source=auth:local-user:backfill",
  ...
  },
  {
    ...
    icon: "ti ti-user-exclamation",
    redirectTo: "/admin/logs?source=auth:guest:backfill",
  ...
  },
  {
    ...
    redirectTo: "/admin/logs?source=auth:reminder:daily",
...
export default function AdminOperations(props: { freeIpaEnabled: boolean }) {
  ...
  const runMutation = mutations.create<{ message?: string; jobId?: string }, OperationConfig>({
    mutation: async (operation) => {
      const response =
        operation.key === "sync"
          ? await apiClient["run-sync"].$post()
          : operation.key === "ipa-backfill"
            ? await apiClient.backfill.ipa.$post()
            : operation.key === "local-user-backfill"
              ? await apiClient.backfill["local-user"].$post()
              : operation.key === "local-guest-backfill"
                ? await apiClient.backfill.guest.$post()
                : await apiClient["run-reminders"].$post();
