...
const SnapshotSaveJobSchema = z.object({
  noteId: z.uuid(),
  targetCursor: z.string().regex(/^\d+-\d+$/),
  reason: z.enum(["periodic", "unload", "shutdown"]),
  requestedAt: z.number().int().nonnegative(),
  sourceNodeId: z.string().min(1),
});

type SnapshotSaveJob = z.infer<typeof SnapshotSaveJobSchema>;
type SnapshotReason = SnapshotSaveJob["reason"];
