                  ...
                  {hasAnyWeather && (
                    <div class="flex items-center justify-center gap-1 mt-0.5 h-4">
                      {dayWeather ? (
                        <>
                          <i
                            class={`ti ti-${dayWeather.icon} text-xs ${weatherApp.service.ui.getAvgTempColorClass(dayWeather.tempMin, dayWeather.tempMax)}`}
                          />
                          <span class="text-[10px] text-dimmed">
                            {weatherApp.service.ui.formatTemp(dayWeather.tempMax)} / {weatherApp.service.ui.formatTemp(dayWeather.tempMin)}
                          </span>
                        </>
                      ) : (
                ...
                <div class="w-12 shrink-0 text-center">
                  ...
                  {dayWeather && (
                    <div class="flex items-center justify-center gap-0.5 text-[10px]">
                      <i
                        class={`ti ti-${dayWeather.icon} ${weatherApp.service.ui.getAvgTempColorClass(dayWeather.tempMin, dayWeather.tempMax)}`}
                      />
                      <span class={weatherApp.service.ui.getTempColorClass(dayWeather.tempMax)}>
                        {weatherApp.service.ui.formatTemp(dayWeather.tempMax)}
                      </span>
                    </div>
                  )}
                </div>
