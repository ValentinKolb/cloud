import { FilterChip, type FilterChipSection, prompts } from "@valentinkolb/cloud/lib/ui";
import { mutation as mutations } from "@valentinkolb/cloud/lib/browser";
import { SearchBar } from "@valentinkolb/cloud/lib/islands";
import { apiClient as settingsClient } from "@/settings/client";
import { apiClient as loggingClient } from "@/logging/client";
...
export default function LogFilterBar(props: Props) {
  ...
  const saveMutation = mutations.create<void, number>({
    mutation: async (days) => {
      const res = await settingsClient[":key{.+}"].$put({ param: { key: "logs.retention_days" }, json: { value: days } });
  ...
  const handleSettings = async () => {
    const res = await settingsClient.index.$get();
    let current = 30;
    if (res.ok) {
      const data = (await res.json()) as { settings: Array<{ key: string; value: unknown }> };
    ...
    const result = await prompts.form({
      ...
      fields: { retention_days: { type: "number" as const, label: "Retention (days)", default: current, min: 1, required: true } },
    });
