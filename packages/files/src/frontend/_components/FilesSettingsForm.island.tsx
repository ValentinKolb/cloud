...
export default function FilesSettingsForm(props: Props) {
  ...
  const changedKeys = createMemo<Array<keyof Initial>>(() => {
    const d = draft();
    return (Object.keys(props.initial) as Array<keyof Initial>).filter(
  ...
  return (
    <div>
      <div class="divide-y divide-zinc-100 dark:divide-zinc-800">
        <SettingsField label="Filegate URL" description="URL of the Filegate storage backend" error={() => fieldErrors()["files.filegate_url"]} changed={() => isChanged("files.filegate_url")}>
          <TextInput
        ...
        <SettingsField label="Filegate Token" description="Authentication token for Filegate. The current value is hidden — leave empty to keep it unchanged." error={() => fieldErrors()["files.filegate_token"]} changed={() => isChanged("files.filegate_token")}>
          <TextInput
        ...
        <SettingsField label="Base Homes" description="Filesystem base path for user home directories" error={() => fieldErrors()["files.base_homes"]} changed={() => isChanged("files.base_homes")}>
          <TextInput
        ...
        <SettingsField label="Base Groups" description="Filesystem base path for group shared directories" error={() => fieldErrors()["files.base_groups"]} changed={() => isChanged("files.base_groups")}>
          <TextInput
        ...
        <SettingsField label="Home Dir Mode" description="Octal Unix permissions for user home directories (e.g. 700)" error={() => fieldErrors()["files.home_dir_mode"]} changed={() => isChanged("files.home_dir_mode")}>
          <TextInput value={() => draft()["files.home_dir_mode"]} onChange={(v) => update("files.home_dir_mode", v)} placeholder="700" />
        </SettingsField>

        <SettingsField label="Home File Mode" description="Octal Unix permissions for files in home directories (e.g. 600)" error={() => fieldErrors()["files.home_file_mode"]} changed={() => isChanged("files.home_file_mode")}>
          <TextInput value={() => draft()["files.home_file_mode"]} onChange={(v) => update("files.home_file_mode", v)} placeholder="600" />
        </SettingsField>

        <SettingsField label="Group Dir Mode" description="Octal Unix permissions for group directories (2770 enables SGID)" error={() => fieldErrors()["files.group_dir_mode"]} changed={() => isChanged("files.group_dir_mode")}>
          <TextInput value={() => draft()["files.group_dir_mode"]} onChange={(v) => update("files.group_dir_mode", v)} placeholder="2770" />
        </SettingsField>

        <SettingsField label="Group File Mode" description="Octal Unix permissions for files in group directories (e.g. 660)" error={() => fieldErrors()["files.group_file_mode"]} changed={() => isChanged("files.group_file_mode")}>
          <TextInput value={() => draft()["files.group_file_mode"]} onChange={(v) => update("files.group_file_mode", v)} placeholder="660" />
        </SettingsField>
      </div>
