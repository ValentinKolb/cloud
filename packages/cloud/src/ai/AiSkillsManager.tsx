...
import PermissionEditor from "../access/PermissionEditor";
...
import type { AiSkill, AiSkillEvent, AiSkillFileStat, AiSkillUserView } from "./skills-store";
                      ...
                      {(userView) => (
                        <span class="flex shrink-0 items-center gap-2" title="Whether the assistant may use this skill in your chats">
                          ...
                          <Switch
                            ...
                            onValueChange={(state) => void setUserState.mutate(state)}
                          />
                        </span>
                      )}
                    </Show>
                ...
                >
                  <span class="flex shrink-0 items-center gap-2" title="Disabled skills disappear from everyone's catalog">
                    ...
                    <Switch
                      ...
                      onValueChange={(enabled) => void toggleEnabled.mutate(enabled)}
                    />
                  </span>
                </Show>
              </div>
      ...
      >
        ...
        <PanelDialog.Body>
          <TextInput
            ...
            value={slug}
            onValueChange={(next) => setSlug(next.toLowerCase())}
            required
            maxLength={64}
        ...
        <PanelDialog.Footer>
          ...
          <div class="flex items-center gap-2">
            <Button type="button" variant="secondary" size="sm" disabled={create.loading()} onClick={() => props.close()}>
              Cancel
            </Button>
            <Button type="submit" variant="primary" size="sm" disabled={create.loading() || !slug().trim()}>
...
function SkillRow(props: {
  ...
  return (
    <li class="flex items-center gap-3 rounded-lg bg-white px-3 py-2.5 [box-shadow:var(--ui-control-bevel)] dark:bg-zinc-900">
      <Button type="button" variant="ghost" class="min-w-0 flex-1 justify-start gap-3 text-left" onClick={props.onOpen}>
      ...
      >
        <Switch
          ...
          onValueChange={(state) => void props.onToggle(state)}
        />
      </span>
    </li>
  );
          ...
          <Match when={tab() === "catalog"}>
            <div class="flex flex-col gap-4">
              ...
              <Show when={props.isAdmin && adminCursor()}>
                <div>
                  <Button type="button" variant="secondary" size="sm" disabled={adminLoading()} onClick={() => void loadAdmin(false)}>
