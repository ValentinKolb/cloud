...
suite("bash tool end-to-end", () => {
  test(
    "reads uploads, writes workspace files, mounts active skills, presents results",
    async () => {
  ...
  test(
    "code-approved workspace skill exposes its scripts",
    async () => {
      ...
      try {
        ...
        if (!approved) throw new Error("Expected code approval to return the skill.");

        await sql`UPDATE ai.skills SET code_approved_hash = 'stale' WHERE id = ${skill.id}::uuid`;
        const staleMount = await buildAiSkillsMountFromSkills([{ ...approved, origin: "workspace", userState: "enabled" }]);
        await expect(staleMount.stat(`/${skill.slug}/scripts/gen.js`)).rejects.toThrow();
        ...
        await aiSkillStore.revokeCode({ skillId: skill.id, actorUserId: adminId });
        const revokedMount = await buildAiSkillsMountFromSkills([{ ...approved, origin: "workspace", userState: "enabled" }]);
        await expect(revokedMount.stat(`/${skill.slug}/scripts/gen.js`)).rejects.toThrow();
      } finally {
        await sql`DELETE FROM ai.skill_events WHERE skill_id = ${skill.id}::uuid`;
        await sql`DELETE FROM ai.skills WHERE id = ${skill.id}::uuid`;
