...
export const ImageCropperDemo = () => {
  ...
  const [aspect, setAspect] = createSignal<"square" | "wide" | "free">("square");
  ...
  const pickImage = async () => {
    setError(null);
    try {
      const nextFile = await showFileDialog({ accept: "image/*" });
      setFile(nextFile);
      setCrop(null);
      setResult(null);
    } catch {
      // Native cancel has no UX cost in the lab.
    }
  };
  ...
  const exportCrop = async () => {
    ...
    setError(null);
    try {
      setResult(await createCroppedImageDataUrl(source, state, { maxWidth: 480, maxHeight: 480, format: "webp", quality: 0.86 }));
    } catch (err) {
  ...
  return (
    <DemoCard
      id="imagecropper"
      chip={{ kind: "component", name: "ImageCropper", from: FROM_UI }}
      description="Interactive image cropper for fixed or free aspect ratios. Drag the crop, resize free crops, zoom with the slider, and rotate right in 90 degree steps."
      code={`const [crop, setCrop] = createSignal<ImageCropState | null>(null);
    ...
    >
      <div class="flex flex-col gap-3">
        <div class="flex flex-wrap items-end gap-2">
          <button type="button" class="btn-primary btn-sm" onClick={pickImage}>
          ...
          <div class="min-w-48">
            <Select
              ...
              value={aspect}
              onChange={(id) => {
                setAspect((id as "square" | "wide" | "free" | null) ?? "square");
                setCrop(null);
                setResult(null);
              }}
              options={[
          ...
          <button type="button" class="btn-input btn-sm" onClick={exportCrop} disabled={!file() || !crop() || busy()}>
            <i class={busy() ? "ti ti-loader-2 animate-spin" : "ti ti-scissors"} />
            <span>{busy() ? "Cropping..." : "Export crop"}</span>
          </button>
        </div>

        <Show when={file()} fallback={<p class="text-xs text-dimmed">Choose an image to try fixed and free crop modes.</p>}>
          <ImageCropper
            source={file()!}
            aspect={cropAspect()}
            previewShape={aspect() === "square" ? "circle" : "rect"}
            disabled={busy()}
            onChange={(state) => {
              setCrop(state);
              setResult(null);
        ...
        <Show when={error()}>
          <div class="info-block-error text-xs">{error()}</div>
        </Show>

        <Show when={result()}>
          <div class="grid gap-2 sm:grid-cols-[minmax(0,auto)_minmax(0,1fr)] sm:items-center">
            <img
              src={result()!}
              alt="Cropped result"
              class="max-h-32 max-w-full rounded-lg object-contain shadow-[var(--theme-shadow-elevated)]"
            />
            <p class="text-xs text-dimmed">Cropped WebP preview generated by the shared image crop helper.</p>
          </div>
        </Show>
      </div>
    </DemoCard>
