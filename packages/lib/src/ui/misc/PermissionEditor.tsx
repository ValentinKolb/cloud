...
import { createMutation } from "@/browser/mutation";
import SegmentedControl from "@/ui/input/SegmentedControl";
...
export default function PermissionEditor(props: PermissionEditorProps) {
  ...
  const updateMut = createMutation<{ accessId: string; permission: PermissionLevel }, { accessId: string; permission: PermissionLevel }>({
    mutation: async (data) => {
  ...
  const revokeMut = createMutation<void, string>({
