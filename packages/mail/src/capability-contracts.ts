...
const CapabilityMailSearchExpressionSchema: z.ZodType<MailSearchExpression> = z.lazy(() =>
  z.discriminatedUnion("type", [
    ...
    mailSearchFolderIdSchema,
    mailSearchAssignedToMeSchema,
    z.object({ type: z.literal("and"), expressions: z.array(CapabilityMailSearchExpressionSchema).min(1).max(20) }).strict(),
    z.object({ type: z.literal("or"), expressions: z.array(CapabilityMailSearchExpressionSchema).min(1).max(20) }).strict(),
    z.object({ type: z.literal("not"), expression: CapabilityMailSearchExpressionSchema }).strict(),
  ]),
);
...
export const ConversationListInputSchema = z
  .object({
    ...
    view: z.enum(["needs_action", "mine", "unassigned", "waiting", "done", "snoozed", "recently_active"]).nullable().optional().describe("Optional saved work queue view."),
    ...PageInputShape,
  })
  .strict();
...
export const ConversationGetDataSchema = z
  .object({
    ...
    collaboration: z
      .object({
        assignee: z.object({ id: UuidSchema, uid: z.string(), displayName: z.string(), avatarHash: NullableTextSchema }).strict().nullable(),
        workStatus: z.enum(["needs_action", "waiting", "done"]),
        snoozedUntil: NullableTimestampSchema,
        revision: z.number().int().positive(),
      })
      .strict(),
    tags: z.array(z.object({ id: UuidSchema, name: z.string(), color: z.string(), revision: z.number().int().positive() }).strict()).max(100),
...
const InlineAttachmentInputSchema = z
  .object({
    ...
    base64: z.string().min(1).max(1_500_000).describe("Base64-encoded content; decoded content is limited to one MiB."),
  })
  .strict();
export const DraftCreateInputSchema = z
  .object({
    ...
    body: z.string().max(2 * 1024 * 1024).default("").describe("Editable message body in the selected format."),
    ...
    attachments: z.array(InlineAttachmentInputSchema).max(10).default([]).describe("Small inline attachments to add to the draft."),
  })
  .strict();
export const DraftUpdateInputSchema = z
  .object({ mailboxId: MailboxIdInputSchema, draftId: DraftIdInputSchema, expectedRevision: ExpectedRevisionInputSchema, draft: draftEditableContentInputSchema.describe("Complete editable draft content replacing the current content.") })
  .strict();
export const DraftDiscardInputSchema = z
  .object({ mailboxId: MailboxIdInputSchema, draftId: DraftIdInputSchema, expectedRevision: ExpectedRevisionInputSchema })
  .strict();
export const DraftAttachmentAddInputSchema = z
  .object({ mailboxId: MailboxIdInputSchema, draftId: DraftIdInputSchema, expectedRevision: ExpectedRevisionInputSchema, attachment: InlineAttachmentInputSchema.describe("Attachment to add.") })
  .strict();
export const DraftAttachmentRemoveInputSchema = z
  .object({ mailboxId: MailboxIdInputSchema, draftId: DraftIdInputSchema, attachmentId: UuidSchema.describe("Draft attachment UUID."), expectedRevision: ExpectedRevisionInputSchema })
  .strict();
...
export const ConversationMoveInputSchema = z
  .object({
    ...
    destination: z.discriminatedUnion("kind", [
      z.object({ kind: z.literal("role"), role: z.enum(["inbox", "archive", "trash", "junk"]) }).strict(),
      z.object({ kind: z.literal("folder"), folderId: UuidSchema }).strict(),
...
export const ConversationMutationItemDataSchema = z
  .object({
    ...
    commands: z
      .array(z.object({ id: UuidSchema, state: z.string().min(1) }).strict())
      .max(500),
  })
  .strict();
...
export const TagDataSchema = z
  .object({ id: UuidSchema, mailboxId: UuidSchema, name: z.string().min(1), color: z.string().regex(/^#[0-9a-f]{6}$/), revision: z.number().int().positive(), createdAt: TimestampSchema, updatedAt: TimestampSchema })
  .strict();
...
export const ConversationTagDataSchema = z.object({ conversationId: UuidSchema, conversationRevision: z.number().int().positive(), tags: z.array(TagDataSchema).max(100) }).strict();
export const TagCreateInputSchema = z
  .object({
    mailboxId: MailboxIdInputSchema,
    name: z.string().trim().min(1).max(80).describe("Tag name."),
    color: z.string().regex(/^#[0-9a-fA-F]{6}$/).describe("Tag color as a six-digit hexadecimal value."),
  })
  .strict();
export const TagUpdateInputSchema = z
  .object({
    ...
    color: z.string().regex(/^#[0-9a-fA-F]{6}$/).optional().describe("Replacement tag color as a six-digit hexadecimal value."),
  })
  .strict()
...
const CollaboratorDataSchema = z.object({ id: UuidSchema, uid: z.string(), displayName: z.string(), avatarHash: NullableTextSchema }).strict();
export const CollaborationDataSchema = z
  .object({ conversationId: UuidSchema, assignee: CollaboratorDataSchema.nullable(), workStatus: z.enum(["needs_action", "waiting", "done"]), snoozedUntil: NullableTimestampSchema, revision: z.number().int().positive() })
  .strict();
export const CollaborationUpdateInputSchema = z
  ...
  .strict()
  .refine((value) => value.assigneeUserId !== undefined || value.workStatus !== undefined || value.snoozedUntil !== undefined, "At least one collaboration field is required");
...
export const ReminderDataSchema = z
  ...
  .strict();
export const ReminderGetInputSchema = z
  .object({ mailboxId: MailboxIdInputSchema, conversationId: ConversationIdInputSchema })
  .strict();
...
export const CommentDataSchema = z
  .object({ id: UuidSchema, conversationId: UuidSchema, body: NullableTextSchema, author: z.object({ kind: z.enum(["user", "service_account", "workflow"]), id: UuidSchema, displayName: z.string(), avatarHash: NullableTextSchema }).strict(), parentCommentId: UuidSchema.nullable(), referencedMessageId: UuidSchema.nullable(), revision: z.number().int().positive(), editedAt: NullableTimestampSchema, deletedAt: NullableTimestampSchema, createdAt: TimestampSchema, updatedAt: TimestampSchema })
  .strict();
...
export const ActivityDataSchema = z
  .object({ id: z.string().min(1), conversationId: UuidSchema.nullable(), actor: z.object({ kind: z.enum(["user", "service_account", "workflow", "system"]), id: UuidSchema.nullable(), displayName: z.string(), avatarHash: NullableTextSchema }).strict(), action: z.string().min(1), outcome: z.enum(["requested", "confirmed", "failed", "reconciled"]), targetType: NullableTextSchema, targetId: NullableTextSchema, metadata: z.record(z.string(), z.unknown()), createdAt: TimestampSchema })
  .strict();
...
const UnsubscribeDataSchema = z
  .object({ kind: z.enum(["one_click", "web", "email"]), href: z.string().min(1).max(2048) })
  .strict();
export const SubscriptionDataSchema = z
  .object({ listKey: z.string().min(1), name: z.string().min(1), address: z.string().min(1), status: z.enum(["active", "requesting", "unsubscribe_requested", "failed"]), unsubscribe: UnsubscribeDataSchema.nullable(), messageCount: z.number().int().nonnegative(), conversationCount: z.number().int().nonnegative(), lastMessageAt: TimestampSchema, lastSubject: z.string(), lastSender: NullableTextSchema, unsubscribeRequestedAt: NullableTimestampSchema, unsubscribeErrorCode: NullableTextSchema })
  .strict();
...
export const SubscriptionUnsubscribeInputSchema = z
  ...
  .strict();
export const SubscriptionUnsubscribeDataSchema = z.object({ listKey: z.string().min(1), status: z.literal("unsubscribe_requested"), requestedAt: TimestampSchema }).strict();
