...
const term: MailSearchExpression = { type: "text", field: "subject", query: "invoice", match: "words" };

describe("mail search validation", () => {
  test("accepts a bounded boolean expression", () => {
    const expression: MailSearchExpression = {
      type: "and",
      expressions: [term, { type: "not", expression: { type: "text", field: "from", query: "spam@example.com", match: "exact" } }],
    };
  ...
  test("rejects expressions deeper than the execution limit", () => {
    ...
    for (let depth = 0; depth < 9; depth += 1) expression = { type: "not", expression };
    expect(validateSearchComplexity(expression).ok).toBe(false);
  });

  test("rejects expressions with more than one hundred nodes", () => {
    const expression: MailSearchExpression = { type: "or", expressions: Array.from({ length: 101 }, () => term) };
    expect(validateSearchComplexity(expression).ok).toBe(false);
  });
  ...
  test("rejects search expressions with excessive aggregate query text", () => {
    const expression: MailSearchExpression = {
      type: "and",
      expressions: Array.from({ length: 11 }, () => ({
        type: "text" as const,
        field: "body" as const,
        query: "x".repeat(500),
