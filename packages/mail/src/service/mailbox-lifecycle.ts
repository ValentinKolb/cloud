...
export type MailboxTransportFence = { remoteResourceId: string; generation: number };
...
export const assertMailboxTransportFence = async (fence: MailboxTransportFence, db: SqlClient = sql): Promise<void> => {
  ...
  `;
  if (!row) {
    throw Object.assign(new Error("Mailbox transport changed during provider work"), { code: "MAILBOX_TRANSPORT_CHANGED" });
  }
};
