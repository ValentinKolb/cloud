...
const normalizeListId = (value: string): { key: string; name: string; address: string } | null => {
...
const allowedExternalHref = (value: string): string | null => {
  try {
...
const oneClickEnabled = (value: string | null): boolean =>
...
export const mailingListMetadata = (
  facts: ReturnType<typeof parseMessageProtocolFacts>,
): {
  listKey: string;
  name: string;
  address: string;
