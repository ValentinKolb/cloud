...
const MailConversationChangedEventSchema = z
  .object({
    ...
    reason: z.enum(["collaboration", "watcher", "comment", "inbound", "threading", "reminder", "local_tag", "reference", "space_link"]),
...
const MailMailboxChangedEventSchema = z
  .object({
    ...
    reason: z.enum(["local_tag", "reference_configuration", "automatic_reply", "scheduled_send", "deleted", "restored"]),
    targetId: z.uuid().nullable(),
    activityId: z.string().min(1).max(128),
    at: z.string().datetime(),
  })
