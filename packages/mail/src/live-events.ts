...
const MailConversationChangedEventSchema = z
  .object({
    ...
    reason: z.enum(["collaboration", "comment", "inbound", "outbound", "threading", "reminder", "local_tag", "reference"]),
...
const MailMailboxChangedEventSchema = z
  .object({
    ...
    reason: z.enum(["local_tag", "reference_configuration", "automatic_reply", "scheduled_send", "folder", "deleted", "restored"]),
    targetId: z.uuid().nullable(),
    activityId: z.string().min(1).max(128),
    at: z.string().datetime(),
  })
