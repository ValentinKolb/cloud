...
export default ssr<AuthContext>(async (c) => {
  ...
  const operatorMailboxes = operationsResult.ok
    ? operationsResult.data.mailboxes
    : [];
    ...
    ? storageResult.data.mailboxes
        ...
        .toSorted((left, right) =>
          sort === "name"
            ...
            : right.logicalTotalBytes - left.logicalTotalBytes ||
              left.mailboxName.localeCompare(right.mailboxName)
        )
    : [];
      ...
      >
        <div class="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p class="text-xs text-dimmed">
              Redacted transport, queue, and projection status without message
              or attachment content.
            </p>
            <p class="text-xs text-dimmed">
              Mail bytes are provider-reported and already include received
              attachments. Publicly shared bytes reference existing files;
        ...
        <StatGrid columns={4}>
          ...
          <StatCell
            ...
            value={
              operationsResult.ok
                ? operationsResult.data.attentionCount
                : "Unavailable"
            }
          ...
          <StatCell
            ...
            value={
              summary
                ? formatBytes(summary.physicalDatabaseBytes)
                : "Unavailable"
            }
          ...
          <StatCell
            label="Blob bytes"
            value={
              summary ? formatBytes(summary.physicalBlobBytes) : "Unavailable"
            }
        ...
        <form
          action="/admin/mail"
          method="get"
          class="paper flex flex-wrap items-end gap-2 p-2"
          role="search"
        >
        ...
        {operationsResult.ok ? (
          <section class="paper overflow-hidden">
            <DataTable
              ...
              empty={
                query
                  ? "No matching active Mail mailboxes."
                  : "No active Mail mailboxes."
              }
              renderCell={({ row, col }) => {
                if (col.id === "mailbox")
                  return (
                ...
                if (col.id === "health")
                  return (
                    <span class="badge">{row.health.replaceAll("_", " ")}</span>
                  );
                if (col.id === "lag")
                  return (
                    <span class="tabular-nums text-secondary">
                      {row.sync.lagSeconds == null
                        ? "Never"
                        : `${row.sync.lagSeconds}s`}
                    </span>
                  );
                if (col.id === "coverage")
                  return (
          ...
          <Placeholder
            state="error"
            variant="panel"
            title="Could not load Mail operations"
            description={operationsResult.error.message}
          />
        )}
        ...
        {storageResult.ok ? (
          <section class="paper overflow-hidden">
            <DataTable
              ...
              empty={
                query
                  ? "No matching reconciled Mail storage data."
                  : "No reconciled Mail storage data."
              }
              renderCell={({ row, col }) => {
                if (col.id === "mailbox")
                  return (
          ...
          <Placeholder
            state="error"
            variant="panel"
            title="Could not load Mail storage"
            description={storageResult.error.message}
          />
        )}
      </div>
    </AdminLayout>
