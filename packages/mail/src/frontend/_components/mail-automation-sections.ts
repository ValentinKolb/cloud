const MAIL_AUTOMATION_SECTIONS = ["overview", "automatic-replies", "sender-rules", "workflows", "references"] as const;

export type MailAutomationSection = (typeof MAIL_AUTOMATION_SECTIONS)[number];
type MailAutomationAdminSection = Extract<MailAutomationSection, "sender-rules" | "workflows" | "references">;

const ADMIN_SECTIONS = new Set<MailAutomationSection>(["sender-rules", "workflows", "references"]);

const isMailAutomationSection = (value: string): value is MailAutomationSection =>
