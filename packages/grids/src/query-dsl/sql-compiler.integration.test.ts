...
import { resolveSavedGqlDashboardQuery } from "../service/dashboard-widget-data";
import * as gqlQueries from "../service/gql-queries";
...
describe("Query DSL Postgres smoke", () => {
  ...
  postgresTest("executes a saved grouped GQL query through the dashboard backend contract", async () => {
    const fixture = await insertDslDbFixture();
    try {
      const created = await gqlQueries.create(
        {
          baseId: fixture.baseId,
          tableId: fixture.orders.id,
          name: "Revenue by status",
          source: `
            from table {${fixture.orders.id}}
            group by Status
      ...
      const result = await resolveSavedGqlDashboardQuery(
        created.data.id,
        { userId: null, userGroups: [], isAdmin: true },
        { baseId: fixture.baseId, limit: 10 },
      );

      expect(result.ok).toBe(true);
      if (!result.ok) throw new Error(result.diagnostics.map((diagnostic) => diagnostic.message).join("; "));
      expect(result.mode).toBe("groups");
      const byStatus = new Map(result.rows.map((row) => [row.values.gk_0, row.values]));
      expect(Number(byStatus.get("Open")?.["*__count"])).toBe(1);
      expect(Number(byStatus.get("Open")?.[`${fixture.amountId}__sum`])).toBe(12.5);
      expect(Number(byStatus.get("Closed")?.["*__count"])).toBe(1);

      const hidden = await resolveSavedGqlDashboardQuery(
        created.data.id,
        { userId: null, userGroups: [], isAdmin: true },
        { baseId: uuid() },
      );
      expect(hidden.ok).toBe(false);
      if (!hidden.ok) expect(hidden.diagnostics[0]?.message).toBe("GQL query not found");
    } finally {
      await cleanupFixture(fixture.baseId);
    }
  });

  postgresTest("gates saved dashboard GQL queries by owner/shared visibility", async () => {
    const fixture = await insertDslDbFixture();
    const ownerId = uuid();
    const otherId = uuid();
    try {
      await sql`
        INSERT INTO auth.users (id, uid, provider, profile, display_name, mail)
        VALUES
          (${ownerId}::uuid, ${`gql-owner-${ownerId}`}, 'local', 'user', 'GQL owner', ${`gql-owner-${ownerId}@example.test`}),
          (${otherId}::uuid, ${`gql-other-${otherId}`}, 'local', 'user', 'GQL other', ${`gql-other-${otherId}@example.test`})
      `;
      ...
      const ownerResult = await resolveSavedGqlDashboardQuery(
        created.data.id,
        { userId: ownerId, userGroups: [] },
        { baseId: fixture.baseId, limit: 10 },
      );
      expect(ownerResult.ok).toBe(true);

      const otherResult = await resolveSavedGqlDashboardQuery(
        created.data.id,
        { userId: otherId, userGroups: [] },
        { baseId: fixture.baseId, limit: 10 },
      );
      expect(otherResult.ok).toBe(false);
      if (!otherResult.ok) expect(otherResult.diagnostics[0]?.message).toBe("GQL query not found");

      const adminResult = await resolveSavedGqlDashboardQuery(
        created.data.id,
        { userId: otherId, userGroups: [], isAdmin: true },
        { baseId: fixture.baseId, limit: 10 },
      );
  ...
  postgresTest("executes a saved dashboard GQL query whose source is a saved view", async () => {
    const fixture = await insertDslDbFixture();
    try {
      const viewId = uuid();
      await sql`
        ...
        VALUES (
          ${viewId}::uuid,
          ${shortId("V")},
          ${fixture.orders.id}::uuid,
          'Status summary',
          ${{
            groupBy: [{ fieldId: fixture.statusId, direction: "asc" }],
      ...
      if (!result.ok) throw new Error(result.diagnostics.map((diagnostic) => diagnostic.message).join("; "));
      expect(result.mode).toBe("groups");
      expect(new Set(result.rows.map((row) => row.values.gk_0))).toEqual(new Set(["Open", "Closed", "Backlog"]));
      const byStatus = new Map(result.rows.map((row) => [row.values.gk_0, Number(row.values["*__count"])]));
      expect(byStatus.get("Open")).toBe(1);
      expect(byStatus.get("Closed")).toBe(1);
      expect(byStatus.get("Backlog")).toBe(1);
    } finally {
      await cleanupFixture(fixture.baseId);
    }
  });

  postgresTest("persists rich canonical query source without ViewQuery coercion", async () => {
    const baseId = uuid();
    const tableId = uuid();
    ...
    try {
      await sql`
        INSERT INTO grids.bases (id, short_id, name)
        VALUES (${baseId}::uuid, ${shortId("B")}, 'GQL queries integration')
      `;
      await sql`
        INSERT INTO grids.tables (id, short_id, base_id, name, position)
        VALUES
          (${tableId}::uuid, ${shortId("T")}, ${baseId}::uuid, 'Orders', 0),
          (${otherTableId}::uuid, ${shortId("T")}, ${baseId}::uuid, 'Other', 1)
      `;
      await sql`
        INSERT INTO grids.fields (id, short_id, table_id, name, type, config, position)
        VALUES (${amountId}::uuid, ${shortId("F")}, ${tableId}::uuid, 'Amount', 'number', '{}'::jsonb, 0)
      `;

      const source = `from table Orders\nselect Amount\nwhere Amount > 0\nlimit 5`;
      const canonicalSource = `from table {${tableId}}\nselect {${amountId}}\nwhere {${amountId}} > 0\nlimit 5`;
      const created = await gqlQueries.create(
        {
          baseId,
          tableId,
      ...
      expect(created.ok).toBe(true);
      if (!created.ok) throw new Error(created.error.message);
      expect(created.data.source).toBe(canonicalSource);

      const listed = await gqlQueries.listForBase({ baseId, userId: null });
      expect(listed.map((item) => item.id)).toEqual([created.data.id]);

      const mismatchedRoot = await gqlQueries.create(
        {
          baseId,
      ...
      expect(mismatchedRoot.ok).toBe(false);
      if (!mismatchedRoot.ok) {
        expect(mismatchedRoot.error.code).toBe("BAD_INPUT");
        expect(mismatchedRoot.error.message).toBe("query root table does not match source");
      }

      const duplicate = await gqlQueries.create(
        {
          baseId,
          tableId,
          name: " joined revenue ",
      ...
      const updatedSource = `from table Orders\ninclude deleted\nlimit 10`;
      const canonicalUpdatedSource = `from table {${tableId}}\nlimit 10\ninclude deleted`;
      const updated = await gqlQueries.update(created.data.id, { source: updatedSource, tableId, name: "All rows" }, null);
      expect(updated.ok).toBe(true);
      if (!updated.ok) throw new Error(updated.error.message);
      expect(updated.data.name).toBe("All rows");
      expect(updated.data.source).toBe(canonicalUpdatedSource);

      const removed = await gqlQueries.remove(created.data.id, null);
      expect(removed.ok).toBe(true);
      expect(await gqlQueries.get(created.data.id)).toBeNull();

      const restored = await gqlQueries.restore(created.data.id, null);
