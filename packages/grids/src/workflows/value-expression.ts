...
export const hasInvalidWorkflowMessageExpression = (message: string): boolean => {
  const withoutExpressions = message.replace(EMBEDDED_EXPRESSION, "");
  return withoutExpressions.includes("${{");
};
