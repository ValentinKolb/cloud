...
const indexTemplate = (template: GridTemplate) => {
  ...
  const fields = new Set(
    template.tables.flatMap((table) =>
      table.fields.map((field) => `${table.key}.${field.key}`)
    )
  );
...
const templateFieldByRef = (template: GridTemplate) => {
  const fields = new Map<
    string,
    GridTemplate["tables"][number]["fields"][number]
  >();
  for (const table of template.tables) {
    for (const field of table.fields)
      fields.set(`${table.key}.${field.key}`, field);
  }
  return fields;
};

const compilerFieldFromTemplate = (
  field: TemplateField
): Parameters<typeof isAggregatable>[0] => ({
  id: field.key,
  shortId: field.key,
  tableId: "template",
  name: field.name,
  ...
  test("form input entries include help text", () => {
    for (const template of templates) {
      for (const form of template.forms ?? []) {
        ...
        expect(Array.isArray(fields), `${template.id}.${form.key} fields`).toBe(
          true
        );
        ...
        for (const entry of fields as Array<Record<string, unknown>>) {
          ...
          expect(
            typeof entry.helpText === "string" &&
              entry.helpText.trim().length > 0,
            `${template.id}.${form.key}.${String(entry.fieldId)} helpText`
          ).toBe(true);

          const inlineFields = (
            entry.inlineCreate as { fields?: unknown } | undefined
          )?.fields;
          if (!Array.isArray(inlineFields)) continue;
          for (const inlineEntry of inlineFields as Array<
            Record<string, unknown>
          >) {
            expect(
              typeof inlineEntry.helpText === "string" &&
                inlineEntry.helpText.trim().length > 0,
              `${template.id}.${form.key}.${String(
                entry.fieldId
              )} inline ${String(inlineEntry.fieldId)} helpText`
            ).toBe(true);
          }
        }
      }
  ...
  test("all internal references resolve", () => {
    for (const template of templates) {
      ...
      for (const ref of refsIn(template)) {
        ...
        expect(
          target.has(ref.key),
          `${template.id} missing ${ref.$ref}:${ref.key}`
        ).toBe(true);
      }

      if (template.defaultDashboard) {
        expect(
          index.dashboards.has(template.defaultDashboard),
          `${template.id} defaultDashboard`
        ).toBe(true);
      }
    }
  });
  ...
  test("each template has meaningful dashboard charts", () => {
    for (const template of templates) {
      const viewsByKey = new Map(
        (template.views ?? []).map((view) => [view.key, view])
      );
      const charts = dashboardCells(template).filter(
        (cell) => cell.kind === "chart"
      );
      expect(charts.length, `${template.id} dashboard charts`).toBeGreaterThan(
        0
      );
      ...
      for (const chart of charts) {
        ...
        expect(
          isRef(viewId) && viewId.$ref === "view",
          `${template.id}.${String(chart.id)} view ref`
        ).toBe(true);
        ...
        expect(
          view,
          `${template.id}.${String(chart.id)} chart view exists`
        ).toBeDefined();
        ...
        expect(
          ...
          `${template.id}.${viewId.key} chart groupBy`
        ).toBe(true);
        expect(
          Array.isArray(query.aggregations) && query.aggregations.length > 0,
          `${template.id}.${viewId.key} chart aggregations`
        ).toBe(true);
        ...
        if (chart.format === "currency") {
          expect(
            firstAggregation?.fieldId,
            `${template.id}.${viewId.key} currency chart value field`
          ).not.toBe("*");
          expect(
            firstAggregation?.agg,
            `${template.id}.${viewId.key} currency chart aggregation`
          ).not.toBe("count");
        }
      }
    }
  ...
  test("template grouped aggregations are backend-compatible", () => {
    for (const template of templates) {
      ...
      for (const view of template.views ?? []) {
        ...
        for (const rawAggregation of aggregationLike as Array<
          Record<string, unknown>
        >) {
          ...
          if (fieldId === "*") {
            expect(agg, `${template.id}.${view.key} "*" aggregate`).toBe(
              "count"
            );
            continue;
          }
          expect(
            isRef(fieldId),
            `${template.id}.${view.key} aggregate field ref`
          ).toBe(true);
          ...
          expect(
            field,
            `${template.id}.${view.key} aggregate field exists`
          ).toBeDefined();
          if (!field) continue;
          expect(
            isAggregatable(
              compilerFieldFromTemplate(field),
              agg as Parameters<typeof isAggregatable>[1],
              false
            ),
    ...
    for (const table of bookshop.tables) {
      for (const field of table.fields) {
        ...
        expect(
          ...
          `bookshop.${table.key}.${field.key} label must not expose snake_case`
        ).toBe(false);
        expect(
          field.name[0],
          `bookshop.${table.key}.${field.key} label should be human-readable`
        ).toBe(field.name[0]?.toUpperCase());
      }
    }
  });
});
