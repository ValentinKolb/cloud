...
describe("isVisibleByAclTiers — most-specific-wins", () => {
  ...
  test("user:none hides shared view", () => {
    expect(
      isVisibleByAclTiers(
        { userRank: 0, groupRank: null, authRank: null, publicRank: null },
        dflt,
      ),
    ).toBe(false);
  });
  ...
  test("public:read + user:none → user wins (hidden)", () => {
    expect(
      isVisibleByAclTiers(
        { userRank: 0, groupRank: null, authRank: null, publicRank: 1 },
        dflt,
      ),
    ).toBe(false);
  });
  ...
  test("public:none + user:read → user wins (visible)", () => {
    expect(
      isVisibleByAclTiers(
        { userRank: 1, groupRank: null, authRank: null, publicRank: 0 },
        { ownerUserId: null, viewerUserId: "user-1" },
      ),
    ).toBe(true);
  });
  ...
  test("group:read + group:none on same tier → deny wins (rank 0 from same-tier-deny aggregation)", () => {
    ...
    expect(
      isVisibleByAclTiers(
        { userRank: null, groupRank: 0, authRank: null, publicRank: null },
        dflt,
      ),
    ).toBe(false);
  });
  ...
  test("group:read + public:none → group wins (visible)", () => {
    expect(
      isVisibleByAclTiers(
        { userRank: null, groupRank: 1, authRank: null, publicRank: 0 },
        dflt,
      ),
    ).toBe(true);
  });
  ...
  test("authenticated:read on shared → visible", () => {
    expect(
      isVisibleByAclTiers(
        { userRank: null, groupRank: null, authRank: 1, publicRank: null },
        dflt,
      ),
    ).toBe(true);
  });
  ...
  test("authenticated:read + group:none → group wins (hidden)", () => {
    expect(
      isVisibleByAclTiers(
        { userRank: null, groupRank: 0, authRank: 1, publicRank: null },
        dflt,
      ),
    ).toBe(false);
  });
