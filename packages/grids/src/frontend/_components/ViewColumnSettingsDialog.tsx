...
type FormulaFormatChoice = "default" | "number" | "percent" | "date" | "progress";
export type ColumnFormatControlsHandle = { value: () => FormatSpec | undefined };

export const openViewColumnSettingsDialog = (args: Args) =>
  dialogCore.open<ViewColumnSettingsResult | null>((close) => <ViewColumnSettingsDialog args={args} close={close} />);
...
function ViewColumnSettingsDialog(props: { args: Args; close: (result: ViewColumnSettingsResult | null) => void }) {
  ...
  return (
    <div class="flex flex-col gap-4">
      <DialogHeader title={`Column — ${props.args.title}`} icon="ti ti-settings" close={() => props.close(null)} />
...
export function ColumnFormatControls(props: {
  ...
  const buildFormat = (): FormatSpec | undefined => {
    ...
    if (fieldType() === "number" || fieldType() === "decimal" || fieldType() === "autonumber") {
      return customNumber()
        ? {
            kind: "decimal",
  ...
  return (
    <div class="flex flex-col gap-4">
      <Show
        ...
        fallback={
          <Show
            when={fieldType() === "number" || fieldType() === "decimal" || fieldType() === "autonumber"}
            fallback={
              <Show
